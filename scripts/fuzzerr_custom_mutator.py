...
def fuzz(buf, add_buf, max_size):
    ...
    # total bits in the buf
    total_bits = len(buf) * 8

    # flip (0-2%) of the bits
    count = random.randint(0, 2) * total_bits // 100
    print(
        f">>>> fuzzer_custom_mutator: flipping {count} bits out of {total_bits} total bits"
    )
    idx_to_flip = random.sample(range(total_bits), count)
